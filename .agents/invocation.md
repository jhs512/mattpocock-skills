...
In this fork, model-invoked is the **default**, in every bucket. A skill is user-invoked only when an agent reaching for it autonomously would be a problem in itself, not merely unhelpful: it mutates the repo's own configuration, or commits work the human has not agreed to. Today that is `setup-matt-pocock-skills` alone. Upstream splits them by role instead (user-invoked orchestrates, model-invoked holds reusable discipline); when a merge from upstream restores `disable-model-invocation` on anything else, take this side.

The rule is not maintained by hand. `scripts/apply-fork-policy.sh` holds the allowlist and enforces it both ways: it strips the flag and the paired `policy` block from every other skill, and puts them back on an allowlisted skill that has lost them. Run it after every merge from upstream, because a new upstream skill arrives with the flag already set, through a clean merge, with no conflict to warn anyone. `--check` reports drift without writing.
...
Bucket `README.md`s and the top-level `README.md` group entries into **User-invoked** and **Model-invoked**. Omit whichever heading has no skills under it rather than leaving it empty.
